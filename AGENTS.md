...
│   │   ├── CommentSection.vue  # Comment list + add-comment input
│   │   ├── PhotoViewer.vue  # Swipeable photo gallery
│   │   └── RichText.vue     # Text with tappable links
│   ├── composables/
...
2. Export singleton class instances for stateful services, or plain functions for stateless utilities
3. Import and use in components/composables

### Adding a shared utility

1. Create or extend a file in `src/utils/`
2. Export pure functions or constants only — no side effects, no imports from services
3. Add a corresponding `.spec.ts` test file
