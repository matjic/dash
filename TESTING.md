...
- **Total Tests**: 183 tests (181 passing, 2 skipped)
- **Test Files**: 12 files
- **Coverage**: Services, models, composables, components, and utilities
...
│   ├── notificationService.spec.ts # Notification scheduling tests
│   ├── recurrenceService.spec.ts  # Recurrence logic tests
│   ├── linkService.spec.ts        # Link parsing tests
│   └── database.sync.spec.ts      # Schema sync validation tests
├── composables/
...
└── utils/
    ...
    └── string.spec.ts             # String utility tests
tests/
└── e2e/
